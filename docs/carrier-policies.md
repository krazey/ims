...
Samsung's carrier JSON does not describe every header generated by its native
IMS stack. Verified exceptions can therefore select
`outgoing_pani_policy=LTE_CELL_IDENTITY` and independently select
`outgoing_visited_network_policy=REGISTRATION_REALM`. The LTE policy reads the
registered packet-service cell identity at call time and is inactive on IWLAN;
it never stores a subscriber's TAC or cell ID in the carrier database.
